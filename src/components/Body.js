  ...
  return (
    <>  
      <div className=" flex justify-center w-full p-5 space-x-12">
        ...
        <button
          className="bg-orange-300 p-2 rounded-md  hover:bg-orange-400"
          onClick={() => {
            const filterpost = listOfRestaurant.filter(
              (res) => res.info.avgRating < 4.3
            );
            setfilteredRestaurant(filterpost);
          }}
        >
        ...
        <input
          type="search"
          className=" w-3/12 p-2  border-2 border-solid border-gray-300 rounded-xl text-orange"
          value={loggedInUser}
      ...
      <div className="flex flex-wrap justify-center list-none font-sans">
        {filteredRestaurant.map((restaurant) => (
          ...
          {
            restaurant.data?.promoted ? (<PromotedCard resData={restaurant}/>) : ( <Card resData={restaurant}/> )
          }
          </Link>
          </li>
        ))}
