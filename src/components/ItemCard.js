    ...
    return (
        <div key={item.card.info.id} className="p-2 w-6/12 mx-auto border-orange-300 border-b-2 flex flex-row items-center justify-center text-left">
          <div className='w-6/12'>
            <span className='font-bold text-xl'> Information : {item.card.info.name} </span><br />
            <span>₹ {item.card.info.price/100.00}</span>
            <p className='text-xs'>{item.card.info.description}</p>
          </div>
          <div className="w-5/12">
            <div className='absolute'>
              <button onClick={() => handleAddItem(item)} className="bg-black text-white cursor-pointer p-1 px-3 rounded-xl ml-16">Add +</button>
            </div>
